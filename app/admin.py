...
class PcmaxAdmin(admin.ModelAdmin):
    ...
    fields = ('name', 'login_id', 'password',   'post_title', 'post_content', 'return_foot_message',
              # 'mail_img','fst_mail',
...
class HappymailAdmin(admin.ModelAdmin):
    list_display = ('name', 'login_id', 'password', 'post_title',)  # 表示するフィールドを指定
    # 編集可能なフィールドを指定（必要に応じて）
    fields = ('name', 'login_id', 'password', 'post_title', 'post_contents', 'return_foot_message',
